...
module {
  ...
  func.func @conv_2d_nhwc_fhwc(%arg0: memref<?x?x?x?xf32>, %arg1: memref<?x?x?x?xf32>, %arg2: memref<?x?x?x?xf32>) {
    ...
    %vec0 = vector.splat %f0 : vector<10xf32>
      ...
      affine.for %idx_h_o = %c0 to %h_o {               
        affine.for %idx_w_o = %c0 to %w_o {          
          %tmp8 = affine.for %idx_h_k = %c0 to %h_k iter_args(%tmp9 = %vec1) -> (vector<6xf32>) {             
            %tmp6 = affine.for %idx_w_k = %c0 to #map(%w_k) iter_args(%tmp7 = %vec1) -> (vector<6xf32>) {
              %1 = arith.muli %idx_w_k, %c2 : index
              %2 = arith.subi %c, %1 : index
              %3 = arith.cmpi sge, %2, %c2 : index
              %in_iter_h = affine.apply #map1 (%idx_h_o, %idx_h_k)
              %in_iter_w = affine.apply #map2 (%idx_w_o, %idx_w_k)
              %tmp10 = scf.if %3 ->vector<6xf32> {
                %input_vec = affine.vector_load %arg0[%idx_n, %in_iter_h, %in_iter_w, %c0] : memref<?x?x?x?xf32>, vector<10xf32>
                %tmp0 = affine.for %idx_f = %c0 to %f iter_args(%tmp1 = %vec1) -> (vector<6xf32>) {
                  %kernel_vec = affine.vector_load %arg1[%idx_f, %idx_h_k, %idx_w_k, %c0] : memref<?x?x?x?xf32>, vector<10xf32>
                  %tmp_vec0 = arith.mulf %kernel_vec, %input_vec : vector<10xf32>
                  %tmp_val = vector.reduction <add>, %tmp_vec0 : vector<10xf32> into f32 
                  %tmp4 = vector.insert %tmp_val, %tmp1[%idx_f] : f32 into vector<6xf32> 
                  affine.yield %tmp4 : vector<6xf32>
                }
                scf.yield %tmp0 : vector<6xf32>
              } else {
                %tmp_4 = arith.muli %c, %2 : index
                %4 = vector.create_mask %tmp_4 : vector<10xi1>
                %input_vec = vector.maskedload %arg0[%idx_n, %in_iter_h, %in_iter_w, %c0], %4, %vec0 : memref<?x?x?x?xf32>, vector<10xi1>, vector<10xf32> into vector<10xf32>
                %tmp0 = affine.for %idx_f = %c0 to %f iter_args(%tmp1 = %vec1) -> (vector<6xf32>) {
                  %kernel_vec = vector.maskedload %arg1[%idx_f, %idx_h_k, %idx_w_k, %c0], %4, %vec0 : memref<?x?x?x?xf32>, vector<10xi1>, vector<10xf32> into vector<10xf32>
                  %tmp_vec0 = arith.mulf %kernel_vec, %input_vec : vector<10xf32>
                  %tmp_val = vector.reduction <add>, %tmp_vec0 : vector<10xf32> into f32 
                  %tmp4 = vector.insert %tmp_val, %tmp1[%idx_f] : f32 into vector<6xf32> 
                  affine.yield %tmp4 : vector<6xf32>
              ...
              %tmp5 = arith.addf %tmp7, %tmp10 : vector<6xf32>
