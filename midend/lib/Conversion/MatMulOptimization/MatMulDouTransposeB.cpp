...
class MatMulDouTransBVecPattern : public ConversionPattern {
public:
  explicit MatMulDouTransBVecPattern(MLIRContext *context,
...
//===----------------------------------------------------------------------===//
// MatMulDouTransBVeccPass
//===----------------------------------------------------------------------===//

/// This is a partial lowering linalg pooling operations to mixture of
/// Affine + Vector operations.
namespace {
class MatMulDouTransBVeccPass
    : public PassWrapper<MatMulDouTransBVeccPass, OperationPass<ModuleOp>> {
public:
  MLIR_DEFINE_EXPLICIT_INTERNAL_INLINE_TYPE_ID(MatMulDouTransBVeccPass)
  ...
  MatMulDouTransBVeccPass() = default;
  MatMulDouTransBVeccPass(const MatMulDouTransBVeccPass &) {}
  explicit MatMulDouTransBVeccPass(int64_t vecSizeParam) {
    vecSize = vecSizeParam;
  }
...
void MatMulDouTransBVeccPass::runOnOperation() {
  ...
  patterns.add<MatMulDouTransBVecPattern>(context, vecSize);
...
namespace mlir {
namespace buddy {
void registerMatMulDouTransBVeccPass() {
  PassRegistration<MatMulDouTransBVeccPass>();
}
} // namespace buddy
} // namespace mlir
